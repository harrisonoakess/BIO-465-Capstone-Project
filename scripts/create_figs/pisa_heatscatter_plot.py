...
# PROJECT_ROOT = Path("/uufs/chpc.utah.edu/common/home/u6073680/Capstone/BIO-465-Capstone-Project")
# PROCESSED_DIR = PROJECT_ROOT / "processed_outputs"
# PLOT_DIR = PROJECT_ROOT / "plots"
# SOURCE_DIR = PROJECT_ROOT / "prep_files" / "output_files"
...
def create_heat_scatter_plot(data: pd.DataFrame, ligand_name_1: str, ligand_name_2: str, log_scale: bool = False):
    x_col = f"{ligand_name_1}_neg_log_molar_affinity_pred_value"
    y_col = f"{ligand_name_2}_neg_log_molar_affinity_pred_value"
    ...
    ax.set_xlabel(f"{clean_ligand_name_1} Predicted Affinity (-log10 M)")
    ax.set_ylabel(f"{clean_ligand_name_2} Predicted Affinity (-log10 M)")
    ax.grid(True)
    # ax.tight_layout()
    ...
    #     plot_file_name = f"heat_scatter_{ligand_name_1}_vs_{ligand_name_2}_log.png"
    ...
    if log_scale:
        ...
        slope, intercept, r_value, p_value, std_err = linear_regression(log_x, log_y)
        m = slope
        b = intercept
        r2 = r_value ** 2
        ...
        plot_file_name = f"pisa_heat_scatter_{ligand_name_1}_vs_{ligand_name_2}_log.png"
        ...
        # ax.title("Experimental vs Predicted Affinity (uM)")
        ax.set_title("Heat Scatter Plot of Predicted Affinities")
        plot_file_name = f"pisa_heat_scatter_{ligand_name_1}_vs_{ligand_name_2}.png"
